...
static void HelpBsp()
{
	const std::vector<HelpOption> options = {
		...
		{"-nolightmaps", "Disable lightmap computation (creates minimal stub)"},
		{"-nolightprobes", "Disable light probe computation (creates single stub probe)"},
		{"-onlyents", "Only update entities in the BSP"},
		{"-patchmeta", "Turn patches into triangle meshes for display"},
		{"-singlelightprobe", "Generate only a single light probe at world center"},
